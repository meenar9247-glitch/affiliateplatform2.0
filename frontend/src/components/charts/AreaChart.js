...
const AreaChart = ({
  ...
  }, [data, dimensions, zoomLevel, panOffset, hoveredPoint, animationProgress]);
  ...
  const drawChart = () => {
    ...
    // Draw areas
    if (stacked) {
      drawStackedAreas(ctx, xScale, yScale, minY);
    } else {
      drawAreas(ctx, xScale, yScale, minY);
    }
    
    // Draw points
    if (pointSize > 0) {
      drawPoints(ctx, xScale, yScale, minY);
    }
  ...
  const drawAreas = (ctx, xScale, yScale, minY) => {
    series.forEach((s, index) => {
      ctx.beginPath();
      
      // Start from bottom
      const firstX = margin.left + xScale / 2;
      const firstY = margin.top + dimensions.height;
      ctx.moveTo(firstX, firstY);
      
      // Draw line
      data.forEach((d, i) => {
        const value = (d[s.key] || 0) * animationProgress;
        const x = margin.left + i * xScale + xScale / 2;
        const y = margin.top + dimensions.height - (value - minY) * yScale;
        
        if (curved) {
          if (i === 0) {
            ctx.lineTo(x, y);
          } else {
            const prevX = margin.left + (i - 1) * xScale + xScale / 2;
            const prevY = margin.top + dimensions.height - ((data[i-1][s.key] || 0) * animationProgress - minY) * yScale;
      ...
      // Complete the area
      const lastX = margin.left + (data.length - 1) * xScale + xScale / 2;
      ctx.lineTo(lastX, margin.top + dimensions.height);
      ctx.closePath();
  ...
  const drawStackedAreas = (ctx, xScale, yScale, minY) => {
    const stackedValues = Array(data.length).fill(0);
    ...
    series.forEach((s, index) => {
      ...
      data.forEach((d, i) => {
        ...
        const x = margin.left + i * xScale + xScale / 2;
        const y = margin.top + dimensions.height - (stackedValues[i] - minY) * yScale;
  ...
  const drawPoints = (ctx, xScale, yScale, minY) => {
    series.forEach((s, index) => {
      data.forEach((d, i) => {
        const value = (d[s.key] || 0) * animationProgress;
        const x = margin.left + i * xScale + xScale / 2;
  ...
  const drawHoverPoint = (ctx, point, xScale, yScale, minY) => {
    ...
    const value = data[dataIndex][series[seriesIndex].key] * animationProgress;
  ...
  const handleMouseMove = (e) => {
    ...
    for (let sIndex = 0; sIndex < series.length; sIndex++) {
      for (let dIndex = 0; dIndex < data.length; dIndex++) {
        const value = data[dIndex][series[sIndex].key] * animationProgress;
        const pointX = margin.left + dIndex * xScale + xScale / 2;
        const pointY = margin.top + dimensions.height - (value - minY) * yScale;
  ...
  const styles = `
    ...
    .area-chart-container.fullscreen {
      position: fixed;
      top: 0;
      left: 0;
      right: 0;
...
chart-title {
  ...
  return (
    <>
      ...
      <div
        ref={containerRef}
        className={`area-chart-container ${isFullscreen ? 'fullscreen' : ''} ${className}`}
...
export const StackedAreaChart = (props) => {
  return <AreaChart {...props} stacked={true} />;
};

// Mini Area Chart (for dashboards)
...
// Sparkline Area Chart (tiny inline chart)
export const SparklineAreaChart = ({
  ...
  return <canvas ref={canvasRef} className="sparkline-area-chart" {...props} />;
};
